...
macro_rules! uint_validator {
    ...
    (
        ...
        impl $crate::view::Validator for $name {
            type Item = $ty;

            fn validate(item: &Self::Item) -> anyhow::Result<()> {
                static ONE_OF: &[$ty] = &[ $($crate::uint_validator!(@one_of_helper $ty; $one_of)),+ ];

                if ONE_OF.contains(item) {
                    Ok(())
                } else {
                    Err(anyhow::anyhow!("Value must be one of {:?}", ONE_OF))
                }
            }
        }
    };
    (
        ...
        impl $crate::view::Validator for $name {
            ...
            fn validate(item: &Self::Item) -> anyhow::Result<()> {
                if *item < $lower {
                    Err(anyhow::anyhow!("Value must be greater than or equal to {}", $lower))
                } else if *item > $upper {
                    Err(anyhow::anyhow!("Value must be less than or equal to {}", $upper))
                } else {
                    Ok(())
                }
            }
